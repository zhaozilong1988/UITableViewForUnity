namespace UIKit
{
	public interface IUIGridViewDataSource : IUITableViewDataSource
	{
		...
		/// </summary>
		int NumberOfColumnAtRowInGridView(UITableView gridView, int rowIndex);

		/// <summary>
		/// Alignment of cells at last row in grid view (table view).
		/// </summary>
		UITableViewAlignment AlignmentOfCellsAtRowInGridView(UITableView grid, int rowIndex);
		...
		/// </summary>
		float WidthForCellAtRowInGridView(UITableView gridView, int rowIndex, int columnIndex, float averageWidthAtRow);
	}
}
