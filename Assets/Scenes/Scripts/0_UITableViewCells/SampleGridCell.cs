...
public class SampleGridCell : UITableViewCell
{
	...
	public void UpdateData(int cellIndex, SampleGridView.Mode mode)
	{
		_editIcon.enabled = mode != SampleGridView.Mode.Normal;
		_indexText.text = cellIndex.ToString();
		var sprite = _monsterSprites[cellIndex % _monsterSprites.Length];
		_icon.sprite = sprite;
		
		switch (mode) {
			case SampleGridView.Mode.Normal:
				break;
			case SampleGridView.Mode.Draggable:
				_editIcon.sprite = _draggableSprite;
				break;
			case SampleGridView.Mode.Deletable:
